...
/**
 * What each option would look like on *your* creature.
 *
 ...
 * one decision — a preview is the user's own appearance with a single field
 * swapped, drawn by `PetRenderer`, the same class the room runs on. There is no
 * icon set to keep in sync, because there are no icons.
 *
 * ```text
 *   your appearance  +  { earType: 'floppy' }  ──→  PetRenderer  ──→  crop to
 *                                                                     the head
 * ```
 *
...
function regionFor(subject: Container, focus: PreviewFocus): Rectangle | null {
  ...
  return padded(squared(region), FOCUS_PAD[focus]);
}
...
export function renderOptionPreview(
  ...
  // The key names everything that changes the picture and nothing that does
  // not, which is what stops the cache from either going stale or missing.
  return renderPreview(`pet:${focus}:${JSON.stringify(shown)}`, () => new PetRenderer(shown).root, {
    size,
    fill: TILE_FILL,
